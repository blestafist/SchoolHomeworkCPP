...
// Returs count of plots with at least 70% grass
...
void FindSymmetryPlotNums (const char (&arr)[NUM_PLOTS][DIMENSIONS][DIMENSIONS], int& id1, int& id2) {
    for (int i = 0; i < NUM_PLOTS; i++) {
        for (int j = i + 1; j < NUM_PLOTS; j++) {
            if (AreIdenticalRotated(arr, i, j)) { id1 = i + 1; id2 = j + 1; return; } 
        }
    }
}


// Searches for the largest area from the upper left corner without obstacles ('X')
...
int main () {
    ...
    FindSymmetryPlotNums(mainArr, id1, id2);
